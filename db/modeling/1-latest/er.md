...
変更点
- フラグ判定していた箇所をテーブル分割で対応する
- 計算によって導出される結果はorder_totalsテーブルで保管する。基本的には計算結果は持たせたくはないが、履歴としてすぐに参照できるように作成
- updateしないテーブルにはcreated_atだけにしている
- order_details の tax_rateは整数値で管理する。アプリケーション側での浮動小数点の扱いでバグる要因を減らすため。updated_atは削除。打ち消しのInsertをすればいいだけにする
- キャンペーンの設計見直し
  - 合計金額に対する割引キャンペーンで開催中は全顧客が一律適用されるという仕様
